...
func NetServiceInit(netSvc i.NetworkServiceInterface) {
	unmarshalHandler := func(next i.UnmarshalMwTaskHandler, in []byte, md i.MethodDescriptorInterface, df i.DataFrameInterface) (out []byte, err error) {
		enc, _ := aes.NewCipher(netSvc.Service_EncriptionKey(), []byte{0x1, 0x2, 0x3, 0x4})
		out, _ = enc.Decrypt(in)
		return
	}
	marshalHandler := func(next i.MarshalMwTaskHandler, in []byte, md i.MethodDescriptorInterface, df i.DataFrameInterface) (out []byte, err error) {
		enc, _ := aes.NewCipher(netSvc.Service_EncriptionKey(), []byte{0x1, 0x2, 0x3, 0x4})
		out = enc.Encrypt(in)
		return
	}
